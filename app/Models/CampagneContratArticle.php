...
class CampagneContratArticle extends Model
{
    ...
    /** Variante pour les campagnes d'enrôlement app mobile (pas de vente de carte). */
    public static function defaultArticlesDefinitionsEnrolement(): array
    {
        return [
            [
                'titre' => 'Article 1 : Objet du contrat',
    ...
    public static function seedDefaultsIfEmpty(int $campagneId, string $type = 'vente_carte'): void
    {
        if (self::query()->where('campagne_id', $campagneId)->exists()) {
            return;
        }
        $definitions = $type === 'enrolement_app'
            ? self::defaultArticlesDefinitionsEnrolement()
            : self::defaultArticlesDefinitions();
